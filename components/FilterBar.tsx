...
export default function FilterBar({
    ...
    onDepartmentChange,
    onDateChange,
    showSearch = true,         // Mặc định true, có thể ẩn input tìm kiếm
}: FilterBarProps) {
    ...
    const handleDateChange = (range: DateRange | undefined) => {
        ...
        if (range?.from && range?.to) {
            const from = format(range.from, "yyyyMMdd").substring(1);// bỏ số 0 đầu
            const to = format(range.to, "yyyyMMdd").substring(1);
            onDateChange(from, to);// truyền lên để fetch dữ liệu
        }
    };
